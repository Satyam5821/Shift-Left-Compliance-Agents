...
def build_prompt(
    ...
    prompt = (
        ...
        + "- When removing a block (e.g., an inner try/catch), use op=replace with the ENTIRE old\n"
        + "  block as old_code and the ENTIRE replacement code as new_code. Do NOT split into\n"
        + "  insert_before + delete.\n"
        + "- NEVER use placeholder text like '...','TODO','nested try block code','old code here'.\n"
        + "  If you cannot copy the exact code from CODE CONTEXT, set code_changes to an empty array.\n"
